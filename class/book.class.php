...
class Book extends CommonObject
{
	...
	/**
	 * @var string 	If permission must be checkec with hasRight('bibliotheque', 'read') and not hasright('mymodyle', 'book', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'bibliotheque';

	/**
	 * @var string 	String with name of icon for book. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'book@bibliotheque' if picto is file 'img/object_book.png'.
	 */
	...
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Book', 'fk_element'=>'fk_book'));
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('bibliotheque', 'book', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('bibliotheque', 'book', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('bibliotheque', 'book_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'book/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'book/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowBook")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Book").'</u>';
	...
	public function getLinesArray()
	{
		...
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_book:=:'.((int) $this->id).')');
...
class BookLine extends CommonObjectLine
{
	 ...
	 */
	public $parent_element = '';		// Example: '' or 'book'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_book'
