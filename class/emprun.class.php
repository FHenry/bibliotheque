...
class Emprun extends CommonObject
{
	...
	public function create(User $user, $notrigger = false)
	{
		if (empty($this->label)) {
			...
			$result = $contact->fetch($this->fk_socpeople);
			if ($result < 0) {
			...
			//$this->label = $contact->lastname . ' '. $contact->firstname;
			$this->label = $contact->getFullName($langs, 1);

			dol_include_once('/bibliotheque/class/book.class.php');
			$book = new Book($this->db);
			$result = $book->fetch($this->fk_book);
			if ($result < 0) {
			...
			$nb_jour_to_add = getDolGlobalInt('BIBLIOTHEQUE_MAX_DAYS_RENT');
			require_once DOL_DOCUMENT_ROOT . '/core/lib/date.lib.php';
			$newtimestap = dol_time_plus_duree($this->date_emprun, $nb_jour_to_add, 'd');
			$this->label .= ' ' . dol_print_date($newtimestap, 'dayhourtext');
			//var_dump($this->label );
			//var_dump(getDolGlobalInt('BIBLIOTHEQUE_MAX_DAYS_RENT'), getDolGlobalString('BIBLIOTHEQUE_MAX_DAYS_RENT'));
			//exit;
		}
