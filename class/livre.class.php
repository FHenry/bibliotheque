...
/**
 * \file        class/livre.class.php
 * \ingroup     bibliotheque
 * \brief       This file is a CRUD class file for Livre (Create/Read/Update/Delete)
 */
...
class Livre extends CommonObject
{
	...
	const STATUS_INACTIF = 3;

	const STATUS_RENT_AVAILABLE = 1;
	...
	//  */
	// public $table_element_line = 'bibliotheque_livreline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_livre';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Livreline';
	...
	//  */
	// protected $childtablesoncascade = array('bibliotheque_livredet');

	// /**
	//  * @var LivreLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	 ...
	 * @return string
	 */
	public function getFieldListBook()
	{
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->bibliotheque->livre->read) {
	...
	public function createFromClone(User $user, $fromid)
	{
		...
		if (property_exists($object, 'fk_status')) {
			$object->status = self::STATUS_DRAFT;
		}
		if (property_exists($object, 'fk_status_rent')) {
			$object->status_rent = self::STATUS_RENT_AVAILABLE;
		}
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (empty($this->isbn)) {
			$this->errors[]=$langs->trans('ErrorFieldRequired', $this->fields['isbn']['label']);
			return -1;
		}
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->bibliotheque->livre->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->bibliotheque->livre->livre_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('LIVRE_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'livre/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'livre/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->bibliotheque->dir_output.'/livre/'.$oldref;
				$dirdest = $conf->bibliotheque->dir_output.'/livre/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->bibliotheque->dir_output.'/livre/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'LIVRE_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		if ($this->status_rent !== self::STATUS_RENT_AVAILABLE) {
			$this->errors[]=$langs->trans('CannotSetToInactifBookIsOut');
			return 0;
		}
		...
		return $this->setStatusCommon($user, self::STATUS_INACTIF, $notrigger, 'LIVRE_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_ACTIF, $notrigger, 'LIVRE_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Livre").'</u>';
		...
		$url = dol_buildpath('/bibliotheque/livre_card.php', 1).'?id='.$this->id;
		...
		$hookmanager->initHooks(array('livredao'));
	...
	{
		return $this->LibStatut($this->status, $this->status_rent, $mode);
	}
	...
	public function LibStatut($status, $status_rent, $mode = 0)
	{
		...
		if (empty($this->labelStatus) || empty($this->labelStatusShort)) {
			...
			$this->labelStatus[self::STATUS_ACTIF] = $langs->trans('Actif');
			$this->labelStatus[self::STATUS_INACTIF] = $langs->trans('Inactif');
			$this->labelStatusShort[self::STATUS_DRAFT] = $langs->trans('Draft');
			$this->labelStatusShort[self::STATUS_ACTIF] = $langs->trans('Actif');
			$this->labelStatusShort[self::STATUS_INACTIF] = $langs->trans('Inactif');
		}

		if (empty($this->labelStatusRent) || empty($this->labelStatusRentShort)) {
			global $langs;
			//$langs->load("bibliotheque@bibliotheque");
			$this->labelStatusRent[self::STATUS_RENT_AVAILABLE] = $langs->trans('Available');
			$this->labelStatusRent[self::STATUS_RENT_OUT] = $langs->trans('Out');
			$this->labelStatusRentShort[self::STATUS_RENT_AVAILABLE] = $langs->trans('Available');
			$this->labelStatusRentShort[self::STATUS_RENT_OUT] = $langs->trans('Out');
		}

		$statusType = 'status'.$status;
		//if ($status == self::STATUS_VALIDATED) $statusType = 'status1';
		if ($status == self::STATUS_INACTIF) {
			$statusType = 'status6';
		}

		$statusRentType = 'status'.$status_rent;
		if ($status_rent == self::STATUS_RENT_OUT) {
			$statusRentType = 'status6';
		}
