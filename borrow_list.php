...
/**
 *    \file       borrow_list.php
 *        \ingroup    bibliotheque
 *        \brief      List page for borrow
 */
...
require_once __DIR__ . '/class/borrow.class.php';
...
$hookmanager->initHooks(array('borrowlist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('bibliotheque', 'borrow', 'write');
	$permissiontodelete = $user->hasRight('bibliotheque', 'borrow', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("bibliotheque")) {
	accessforbidden('Module bibliotheque not enabled');
}
...
//$help_url = "EN:Module_Borrow|FR:Module_Borrow_FR|ES:Módulo_Borrow";
$help_url = '';
$title = $langs->trans("Borrows");
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: " . dol_buildpath('/bibliotheque/borrow_card.php', 1) . '?id=' . $id);
	exit;
}
...
if ($frompage == 'contact') {
	if ($search['fk_socpeople'] > 0) {
		// Si edition contact deja existant
		$contact = new Contact($db);
		$res = $contact->fetch($search['fk_socpeople']);
		if ($res < 0) {
...
if (!empty($frompage)) {
		$param .= '&frompage=' . urlencode($frompage);
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/bibliotheque/borrow_card.php', 1) . '?action=create&backtopage=' . urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_' . $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendBorrowRef";
$modelmail = "borrow";
$objecttmp = new Borrow($db);
