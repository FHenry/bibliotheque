...
dol_include_once('/bibliotheque/lib/bibliotheque_emprunt.lib.php');
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->hasRight('bibliotheque', 'emprunt', 'read');
	$permissiontoadd = $user->hasRight('bibliotheque', 'emprunt', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
	$permissiontodelete = $user->hasRight('bibliotheque', 'emprunt', 'delete') || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	$permissionnote = $user->hasRight('bibliotheque', 'emprunt', 'write'); // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->hasRight('bibliotheque', 'emprunt', 'write'); // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
$upload_dir = $conf->bibliotheque->multidir_output[isset($object->entity) ? $object->entity : 1].'/emprunt';
...
if (empty($reshook)) {
	$backurlforlist = dol_buildpath('/bibliotheque/emprunt_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/bibliotheque/emprunt_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}

	$triggermodname = 'BIBLIOTHEQUE_MYOBJECT_MODIFY'; // Name of trigger action code to execute when we modify record
	...
	$triggersendname = 'BIBLIOTHEQUE_MYOBJECT_SENTBYMAIL';
...
if ($action == 'create') {
	$title = $langs->trans("NewObject", $langs->transnoentitiesnoconv("Emprunt"));
}
$help_url = '';
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	print dol_get_fiche_head($head, 'card', $langs->trans("Emprunt"), -1, $object->picto, 0, '', '', 0, '', 1);

	$formconfirm = '';

	// Confirmation to delete (using preloaded confirm popup)
	if ($action == 'delete' || ($conf->use_javascript_ajax && empty($conf->dol_use_jmobile))) {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteEmprunt'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 'action-delete');
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/bibliotheque/emprunt_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	*/
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			print $formfile->showdocuments('bibliotheque:Emprunt', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/bibliotheque/emprunt_agenda.php', 1).'?id='.$object->id);
