...
/**
 *	Class to manage PDF template standard_livre
 */
class pdf_standard_livre extends ModelePDFLivre
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		/*
		if (!empty($conf->global->MAIN_GENERATE_LIVRE_WITH_PICTURE))
		{
		...
		*/
		...
		if ($conf->bibliotheque->dir_output.'/livre') {
			$object->fetch_thirdparty();

			// Definition of $dir and $file
			if ($object->specimen) {
				$dir = $conf->bibliotheque->dir_output.'/livre';
				$file = $dir."/SPECIMEN.pdf";
			} else {
				$objectref = dol_sanitizeFileName($object->ref);
				$dir = $conf->bibliotheque->dir_output.'/livre/'.$objectref;
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		if ($object->statut == $object::STATUS_DISPONIBLE && (!empty($conf->global->FACTURE_DRAFT_WATERMARK))) {
		...
		if ($object->statut == $object::STATUS_DISPONIBLE) {
