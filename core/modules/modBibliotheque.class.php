...
class modBibliotheque extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		*/
		...
		$this->rights[$r][0] = $this->numero . sprintf("%02d", $r + 1); // Permission id (must not be already used)
		$this->rights[$r][1] = 'bibliReadBorrow'; // Permission label
		$this->rights[$r][4] = 'borrow';
		$this->rights[$r][5] = 'read'; // In php code, permission will be checked by test if ($user->rights->bibliotheque->book->read)
		$r++;
		$this->rights[$r][0] = $this->numero . sprintf("%02d", $r + 1); // Permission id (must not be already used)
		$this->rights[$r][1] = 'bibliCreateUpdBorrow'; // Permission label
		$this->rights[$r][4] = 'borrow';
		$this->rights[$r][5] = 'write'; // In php code, permission will be checked by test if ($user->rights->bibliotheque->book->write)
		$r++;
		$this->rights[$r][0] = $this->numero . sprintf("%02d", $r + 1); // Permission id (must not be already used)
		$this->rights[$r][1] = 'bibliDeleteBorrow'; // Permission label
		$this->rights[$r][4] = 'borrow';
		$this->rights[$r][5] = 'delete'; // In php code, permission will be checked by test if ($user->rights->bibliotheque->book->delete)
		$r++;
		...
		*/
		...
		$r = 1;
		/* BEGIN MODULEBUILDER EXPORT BOOK */
		/*
		...
		$this->export_icon[$r]='book@bibliotheque';
		...
		$this->export_sql_end[$r]  =' FROM '.MAIN_DB_PREFIX.'book as t';
		//$this->export_sql_end[$r]  =' LEFT JOIN '.MAIN_DB_PREFIX.'book_line as tl ON tl.fk_book = t.rowid';
		$this->export_sql_end[$r] .=' WHERE 1 = 1';
		$this->export_sql_end[$r] .=' AND t.entity IN ('.getEntity('book').')';
		$r++; */
		/* END MODULEBUILDER EXPORT BOOK */

		// Imports profiles provided by this module
		$r = 1;
		/* BEGIN MODULEBUILDER IMPORT BOOK */
		/*
		$langs->load("bibliotheque@bibliotheque");
		$this->import_code[$r]=$this->rights_class.'_'.$r;
		$this->import_label[$r]='BookLines';	// Translation key (used only if key ExportDataset_xxx_z not found)
		$this->import_icon[$r]='book@bibliotheque';
		...
		$this->import_convertvalue_array[$r] = array(
			't.ref' => array(
				'rule'=>'getrefifauto',
				'class'=>(empty($conf->global->BIBLIOTHEQUE_BOOK_ADDON) ? 'mod_book_standard' : $conf->global->BIBLIOTHEQUE_BOOK_ADDON),
				'path'=>"/core/modules/commande/".(empty($conf->global->BIBLIOTHEQUE_BOOK_ADDON) ? 'mod_book_standard' : $conf->global->BIBLIOTHEQUE_BOOK_ADDON).'.php'
				'classobject'=>'Book',
	...
	public function init($options = '')
	{
		...
		$result1=$extrafields->addExtraField('nb_borrow', "bibliNbBorrow", 'int', 1,  3, 'contact',   0, 0, '', '', 1, '', 5, 0, '', '', 'bibliotheque@bibliotheque', '$conf->bibliotheque->enabled');
